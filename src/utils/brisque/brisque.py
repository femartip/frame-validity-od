...
class Brisque:
    ...
    def __init__(
        self,
        image: typing.Union[PIL.Image.Image, np.ndarray],
        kernel_size: int = 7,
        sigma: float = 7 / 6,
    ):
    ...
    def local_deviation(self):
        if self._local_deviation is None:
            sigma = np.square(self.image)
            sigma = scipy.signal.convolve2d(sigma, self.kernel, "same")
            self._local_deviation = np.sqrt(
                np.abs(np.square(self.local_mean) - sigma)
            )
    ...
    def calculate_features(self, mscn_type: MscnType):
        ...
        if mscn_type == MscnType.mscn:
            var = np.mean(
                [np.square(agg.sigma_left), np.square(agg.sigma_right)]
            )
            return np.array([agg.alpha, var])

        return np.array(
            [
                agg.alpha,
                agg.mean,
                np.square(agg.sigma_left),
                np.square(agg.sigma_right),
            ]
        )


def scale_features(features: np.ndarray) -> np.ndarray:
    _min = np.array(scale_parameters["min_"])
    _max = np.array(scale_parameters["max_"])
    return -1 + (2.0 / (_max - _min) * (features - _min))


def calculate_features(image: PIL.Image, kernel_size, sigma) -> np.ndarray:
    ...
    features = np.concatenate([brisque.features, downscaled_brisque.features])
