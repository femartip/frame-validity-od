...
def run_yolo(model: YOLO, image_root: str, output_dir: str, output_name : str | None = None) -> None:
    detections_val = {}
    iou_vals = []
    lrp_vals = []
...
def run_rf_detr(model: RFDETRBase, image_root: str, output_dir: str, output_name: str | None = None) -> None:
    detections_val = {}
    iou_vals = []
    lrp_vals = []
    image_files = list_images(image_root)
...
def run_faster_rcnn(predictor: DefaultPredictor, dataset: list[dict], output_dir: str, output_name: str | None = None) -> None:
    detections_val = {}
    iou_vals = []
    lrp_vals = []
...
def main():
    ...
    if args.model == "yolo":
        ...
        if run_train:
            ...
            run_yolo(model, "./data/zod_yolo/images/train", "./results/yolo/")
        if run_val:
            print(f"Processing YOLO val images from ./data/zod_yolo/images/val/")
            run_yolo(model, './data/zod_yolo/images/val/', './results/yolo/', f"detections.json")
    ...
    # Faster RCNN logic
        ...
        if run_train:
            ...
            run_faster_rcnn(predictor, train_dataset, "results/faster-rcnn")
        if run_val:
            ...
            run_faster_rcnn(predictor, val_dataset, "results/faster-rcnn", "detections.json")
    ...
    # Rfdetr logic
        ...
        if run_train:
            ...
            run_rf_detr(model, './data/zod_yolo/images/train/', "results/rf-detr")
        if run_val:
            print(f"Processing RF-DETR val images from {args.yolo_val}")
            run_rf_detr(model, './data/zod_yolo/images/val/', "results/rf-detr", "detections.json")
